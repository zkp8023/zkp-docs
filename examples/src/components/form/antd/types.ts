// 表单配置类型
import type { FormItemProps, FormProps, RowProps } from 'ant-design-vue'
import type { CSSProperties } from 'vue'
/**
 * 表单配置名称 直接在组件库中截取el后半部分
 *  preview 为详情或预览，指定表单控件类型为preview后不会展示表单控件
 */
export type AntdType = 'auto-complete' | 'cascader' | 'checkbox' | 'checkbox-group' | 'checkbox-button' | 'color-picker' |
'date-picker' | 'range-picker' | 'input' | 'input-number' | 'radio' | 'radio-group' | 'radio-button' | 'rate' |
'select' | 'select-option' | 'slider' | 'switch' | 'time-picker' | 'time-range-picker' | 'time-select' | 'upload' |
'transfer' | 'tree-select' | 'preview'
...
export interface IFormSchema {
  // 传入detail字段，整体为详情展示
  form: Partial<FormProps & { detail?: boolean; scrollToFirstError: any }>
  items: IColumn[]
